...
/*   Updated: 2024/09/05 18:17:17 by pwojnaro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*read_and_store(int fd, int *bytes_read, char *line)
{
	...
	if (!line)
		line = ft_calloc(1, sizeof(char));
	if (!line)  // Handle allocation failure
		return (NULL);
	storage = ft_calloc(BUFFER_SIZE + 1, sizeof(char));
	if (!storage)
		return (free(line), NULL);  // Free line if storage allocation fails
	while (*bytes_read > 0)
	{
		*bytes_read = read(fd, storage, BUFFER_SIZE);
		if (*bytes_read == -1)
			return (free(storage), free(line), NULL);  // Free both storage and line on error
		storage[*bytes_read] = '\0';
		leftover = ft_strjoin(line, storage);
		if (!leftover)  // Handle strjoin failure
...
char	*allocate_new_line(char *line, int bytes_read)
{
	...
	i = 0;
	if (!line[bytes_read])  // If we've reached the end of the line
	{
		free(line);
		return (NULL);
	}
	...
	if (!new_line)
		return (free(line), NULL);  // Free line if allocation fails
	bytes_read++;
	while (line[bytes_read])
	{
		new_line[i] = line[bytes_read];
